...
;// CONCATENATED MODULE: ./scripts/options/options-controller.ts
...
class OptionsController {
    constructor() {
        this.ipShowList = undefined;
        this.ipUserAllowedHosts = new Array();
        this.showLoggingToggle = currentBrowser === browser_Browser.firefox;
        this.onDataChangedListeners = new Array();
        this.extensionPortController = new ExtensionPortController(hostConstants.optionsPortName);
        this.extensionPortController.registerMessagePayloadHandler(MessageType.optionsDataResponseV22, message => this.onData(message));
        this.extensionPortController.connect();
        this.loggingEnabled = logger.isEnabled;
        logger.enabledChanged.registerEventHandler(enabled => this.onLoggingEnabledChange(enabled));
    }
    onData(newData) {
        const newUserAllowedHosts = deduplicateAndSort(newData.ipUserAllowedHosts);
        const same = newData.ipShowList === this.ipShowList &&
            compareArrays(newUserAllowedHosts, this.ipUserAllowedHosts);
        if (!same) {
            log_log("OptionsController: options data changed");
            this.ipShowList = newData.ipShowList;
            this.ipUserAllowedHosts = newUserAllowedHosts;
            this.notifyListeners();
        }
    }
    onLoggingEnabledChange(enabled) {
        if (this.loggingEnabled !== enabled) {
            log_log("OptionsController: logging enabled status changed");
    ...
    notifyListeners() {
        var _a;
        for (const listener of this.onDataChangedListeners) {
            listener((_a = this.ipShowList) !== null && _a !== void 0 ? _a : false, this.ipUserAllowedHosts, this.showLoggingToggle, this.loggingEnabled);
        }
    }
    registerOnDataChangedListener(listener) {
        this.onDataChangedListeners.push(listener);
    }
    changeHostnameStatus(changedHostname, allowInput) {
        if (allowInput) {
            this.ipUserAllowedHosts = deduplicateAndSort([...this.ipUserAllowedHosts, changedHostname]);
        }
        else {
            this.ipUserAllowedHosts = this.ipUserAllowedHosts.filter(hostname => hostname !== changedHostname);
        }
        this.extensionPortController.sendMessage(MessageType.phishingHostStatusChangeV22, new PhishingHostStatusChangeV22([changedHostname], allowInput));
    }
...
;// CONCATENATED MODULE: ../lib/common/slider-switch.ts

class SliderSwitch {
    static make(itemName, isChecked, enabled, checkBoxId, onClick) {
        const label = document.createElement("label");
        label.className = "slider-label";
        const checkbox = document.createElement("input");
        checkbox.type = "checkbox";
        if (maybe_some(checkBoxId)) {
            checkbox.id = checkBoxId;
        }
        checkbox.checked = isChecked;
        checkbox.dataset.itemName = itemName;
        checkbox.disabled = !enabled;
        if (maybe_some(onClick)) {
            checkbox.addEventListener("click", onClick);
        }
        label.appendChild(checkbox);
        const stateSpan = document.createElement("span");
        stateSpan.className = "slider-state";
        label.appendChild(stateSpan);
        const onSpan = document.createElement("span");
        onSpan.className = "slider-on";
        stateSpan.appendChild(onSpan);
        const offSpan = document.createElement("span");
        offSpan.className = "slider-off";
        stateSpan.appendChild(offSpan);
        const toggleSpan = document.createElement("span");
        toggleSpan.className = "slider-toggle";
        stateSpan.appendChild(toggleSpan);
        return label;
    }
}
...
;// CONCATENATED MODULE: ./scripts/options/options-view.ts





const kCheckboxPrefix = "id-checkbox-";
const kLabelPrefix = "id-label-";
var OptionsPageIds;
(function (OptionsPageIds) {
    OptionsPageIds["statusNotice"] = "status-notice";
    OptionsPageIds["generalSection"] = "general-section";
    OptionsPageIds["loggingToggleTable"] = "logging-toggle-table";
    OptionsPageIds["identityProtectionSection"] = "identity-protection-section";
    OptionsPageIds["hostTable"] = "host-table";
    OptionsPageIds["allowedDomainsSectionHeader"] = "allowed-domains-section-header";
    OptionsPageIds["noAllowedDomain"] = "no-allowed-domain";
})(OptionsPageIds || (OptionsPageIds = {}));
class OptionsView {
    constructor(controller) {
        this.controller = controller;
        this.displayedHosts = 0;
        controller.registerOnDataChangedListener((showList, userAllowedHosts, showLoggingToggle, userEnabledLogging) => this.onDataChanged(showList, userAllowedHosts, showLoggingToggle, userEnabledLogging));
        const loggingToggleText = getI18n(I18nMessages.optionsLoggingToggle);
        const idStr = "logging-toggle";
        this.loggingToggleCheckboxId = kCheckboxPrefix + idStr;
        this.appendToTable(OptionsPageIds.loggingToggleTable, loggingToggleText, idStr, event => {
            const element = event.target;
            if (element !== null) {
                controller.configureLogging(element.checked);
            }
        });
    }
    setupPage(ipShowList, ipNoAllowedDomains, showLoggingToggle, loggingEnabled) {
        let id;
        const hasOptions = ipShowList || showLoggingToggle;
        if (!hasOptions) {
            id = OptionsPageIds.statusNotice;
            setElementTextContent(window, id, getI18n(I18nMessages.optionsNoOptions));
            doDisplay(window, id);
            return;
        }
        if (showLoggingToggle) {
            id = OptionsPageIds.generalSection;
            doDisplay(window, id);
            const loggingToggle = window.document.getElementById(this.loggingToggleCheckboxId);
            if (loggingToggle !== null) {
                loggingToggle.checked = loggingEnabled;
            }
        }
        if (ipShowList) {
            id = OptionsPageIds.identityProtectionSection;
            doDisplay(window, id);
            id = OptionsPageIds.allowedDomainsSectionHeader;
            setElementTextContent(window, id, getI18n(I18nMessages.ipOptionsAllowedDomainsSectionHeader));
            id = OptionsPageIds.noAllowedDomain;
            if (ipNoAllowedDomains) {
                setElementTextContent(window, id, getI18n(I18nMessages.ipOptionsAllowedDomainsEmpty));
                doDisplay(window, id);
            }
        }
    }
    clearPage() {
        doNotDisplay(window, OptionsPageIds.statusNotice);
        doNotDisplay(window, OptionsPageIds.generalSection);
        doNotDisplay(window, OptionsPageIds.identityProtectionSection);
        doNotDisplay(window, OptionsPageIds.noAllowedDomain);
        const hostTable = window.document.getElementById(OptionsPageIds.hostTable);
        if (hostTable !== null) {
            for (let i = 0; i < this.displayedHosts; ++i) {
                const lastChild = hostTable.lastChild;
                if (lastChild !== null) {
                    hostTable.removeChild(lastChild);
                }
                else {
                    break;
                }
    ...
    appendToTable(tableId, text, idStr, onClick) {
        var _a;
        const row = document.createElement("tr");
        (_a = document.getElementById(tableId)) === null || _a === void 0 ? void 0 : _a.appendChild(row);
        const labelCell = document.createElement("td");
        labelCell.textContent = text;
        labelCell.id = kLabelPrefix + idStr;
        labelCell.align = "left";
        row.appendChild(labelCell);
        const checkBoxCell = document.createElement("td");
        checkBoxCell.align = "center";
        checkBoxCell.appendChild(SliderSwitch.make(text, true, true, kCheckboxPrefix + idStr, onClick));
        row.appendChild(checkBoxCell);
    }
    appendToHostsTable(hostname) {
        this.appendToTable(OptionsPageIds.hostTable, hostname, hostname, event => this.onToggleHost(event));
        this.displayedHosts += 1;
    }
    updateLabelStyle(hostname, crossOut) {
        const labelId = kLabelPrefix + hostname;
    ...
    onToggleHost(event) {
        const element = event.target;
        if (element !== null) {
            const allowInput = element.checked;
            const hostname = element.dataset.itemName;
            if (maybe_some(hostname)) {
                this.controller.changeHostnameStatus(hostname, allowInput);
                this.updateLabelStyle(hostname, !allowInput);
            }
        }
    }
    onDataChanged(ipShowList, ipUserAllowedHosts, showLoggingToggle, loggingEnabled) {
        this.clearPage();
        this.setupPage(ipShowList, isEmpty(ipUserAllowedHosts), showLoggingToggle, loggingEnabled);
        ipUserAllowedHosts.forEach(hostname => this.appendToHostsTable(hostname));
    }
}

;// CONCATENATED MODULE: ./scripts/options/main.ts
...
function main() {
    ...
    const view = new OptionsView(controller);
}
