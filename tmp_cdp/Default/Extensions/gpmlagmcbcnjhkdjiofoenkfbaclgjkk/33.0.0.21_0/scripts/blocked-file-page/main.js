...
;// CONCATENATED MODULE: ./scripts/blocked-file-page/blocked-file-controller.ts
...
class BlockedFileController {
    constructor(document, fileURL) {
        ...
        this.extensionPortController = new ExtensionPortController(hostConstants.blockedFilePagePortName, tabId => this.onExtensionReady());
        this.extensionPortController.registerMessagePayloadHandler(MessageType.blockedFileResponseV1, (message) => this.handleResponse(message));
        this.extensionPortController.connect();
        setTimeout(() => {
            if (!this.hadResponse) {
                log_logError(`Timed out waiting for a response. Will proceed to ${URLToString(fileURL)}`);
                this.navigateToFile(this.fileURL);
            }
        }, 30000);
    }
    navigateToFile(fileURL) {
        navigateDocument(this.document, fileURL);
    }
    handleResponse(response) {
        this.hadResponse = true;
        const isTrusted = response.isTrusted;
        const fileURL = url_utils_parseUrl(response.fileUrlSpec);
        if (maybe_none(fileURL)) {
            return;
        }
        if (isTrusted) {
    ...
    sendRequest() {
        this.extensionPortController.sendMessage(MessageType.blockedFileRequestV1, new BlockedFileRequestV1(URLToString(this.fileURL)));
    }
    onExtensionReady() {
        this.sendRequest();
    }
...
;// CONCATENATED MODULE: ./scripts/blocked-file-page/main.ts
...
function parseOptions(window) {
    ...
    const fileURL = findURLDocumentQueryParam(documentURL, OptionNames.fileUrlSpec);
    if (maybe_none(fileURL)) {
        return undefined;
    }
...
function main(window) {
    ...
    if (maybe_none(options)) {
        return;
    }
    const document = window.document;
    document.title = URLToString(options.fileURL);
    const controller = new BlockedFileController(document, options.fileURL);
}
window.onload = (event) => {
    main(window);
};
