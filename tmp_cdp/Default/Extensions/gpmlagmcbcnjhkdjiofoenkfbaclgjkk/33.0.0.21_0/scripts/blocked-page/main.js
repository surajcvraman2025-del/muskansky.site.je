...
function findDocumentUrl(window) {
    return url_utils_parseUrl(window.location.toString());
}
...
;// CONCATENATED MODULE: ./scripts/blocked-page/url-parser.ts
...
class Options {
    constructor(navigateToURL, blockedURL, contentType, rememberDecisionsDefault, learnMoreURL, launchedURL, showLaunchedPageButtons) {
        ...
        this.showLaunchedPageButtons = showLaunchedPageButtons;
    }
}
function parseUrlOptions(window) {
    const documentUrl = findDocumentUrl(window);
    const navigateToURL = findURLDocumentQueryParam(documentUrl, OptionNames.navigateToSpec);
    const blockedURL = findURLDocumentQueryParam(documentUrl, OptionNames.blockedSpec);
    const contentType = findDocumentQueryParam(documentUrl, OptionNames.contentType);
    const rememberDecisionsDefaultQueryParam = findDocumentQueryParam(documentUrl, OptionNames.rememberDecisionsDefault);
    const learnMoreURLQueryParam = findDocumentQueryParam(documentUrl, OptionNames.learnMoreURL);
    const launchedURL = findURLDocumentQueryParam(documentUrl, OptionNames.launchedSpec);
    const showLaunchedPageButtonsQueryParam = findDocumentQueryParam(documentUrl, OptionNames.showLaunchedPageButtons);
    const rememberDecisionsDefault = rememberDecisionsDefaultQueryParam === "true";
    const showLaunchedPageButtons = showLaunchedPageButtonsQueryParam === "true";
    let learnMoreURL = undefined;
    if (maybe_some(learnMoreURLQueryParam)) {
        const url = url_utils_parseUrl(learnMoreURLQueryParam);
        if (maybe_some(url)) {
            const urlEncoder = new UrlEncoder(url);
            urlEncoder.addQueryParam("locale", chrome.i18n.getUILanguage());
            learnMoreURL = urlEncoder.encodeUrl();
        }
    }
    return new Options(navigateToURL, blockedURL, contentType, rememberDecisionsDefault, learnMoreURL, launchedURL, showLaunchedPageButtons);
}

;// CONCATENATED MODULE: ./scripts/blocked-page/model.ts
class Model {
    constructor(navigateToURL, blockedURL, contentType, rememberDecisionsDefault, learnMoreURL, launchedURL, showLaunchedPageButtons) {
        this.navigateToURL = navigateToURL;
        this.blockedURL = blockedURL;
...
const unknownContentType = (/* unused pure expression or super */ null && (undefined));
var ContentType;
...
;// CONCATENATED MODULE: ./scripts/blocked-page/controller.ts
...
class Controller {
    constructor(model, show, updateViewAfterBlocking) {
        ...
        this.extensionPortController = new ExtensionPortController(hostConstants.blockedPagePortName, tabId => this.onExtensionReady(tabId));
        this.extensionPortController.connect();
        if (parseRawContentType(this.contentType) == ContentType.pdf) {
            this.model.hostnameExplanation = getI18n(I18nMessages.blockedPDFPageTitle);
            this.model.blockedPageOpenedSecureExplanation = getI18n(I18nMessages.blockedPDFPageOpenedSecureExplanation);
            this.model.launchedPageOpenedSecureExplanation = getI18n(I18nMessages.launchedPDFPageOpenedSecureExplanation);
        }
        else {
            this.model.hostnameExplanation = getI18n(I18nMessages.blockedLinkPageTitle);
            this.model.blockedPageOpenedSecureExplanation = getI18n(I18nMessages.blockedLinkPageOpenedSecureExplanation);
            this.model.launchedPageOpenedSecureExplanation = getI18n(I18nMessages.launchedLinkPageOpenedSecureExplanation);
        }
    }
    onExtensionReady(tabId) {
        chrome.tabs.query({}, tabs => {
            this.model.tabCount = tabs.length;
            this.model.tabId = tabId;
            this.show();
        });
    }
    navigateToUrl(window) {
        ...
        if (maybe_some(url)) {
            ...
            navigateDocument(document, url);
        }
    }
    trustUrl(window) {
        const trustUrl = this.model.trustUrl;
        const rememberTrustDecision = this.model.rememberTrustDecision;
        log_log(`BlockedPageController.trustUrl: ${string_utils_toString({
            trustUrl: trustUrl,
            rememberDecision: rememberTrustDecision,
            url: URLToString(this.blockedURL)
        })}`);
        const payload = new TrustUrlV8(URLToString(this.navigateToURL), URLToString(this.blockedURL), trustUrl, rememberTrustDecision, this.contentType);
        this.extensionPortController.sendMessage(MessageType.trustUrlV8, payload);
        if (trustUrl) {
    ...
    onCloseTab() {
        if (maybe_some(this.model.tabId)) {
            chrome.tabs.remove(this.model.tabId);
        }
    }
    onTrustUrl(window) {
...
;// CONCATENATED MODULE: ./scripts/blocked-page/view.ts
...
(function (Ids) {
    ...
    Ids["goBackButton"] = "go-back-button";
    Ids["closeTabButton"] = "close-tab-button";
})(Ids || (Ids = {}));
class View {
    constructor(controller, model, window) {
    ...
    setHostname(window, hostname) {
        ...
        setElementTextContent(window, Ids.hostname, hostname);
    }
    setWarningTooltip(window, message) {
        setTooltipText(window, Ids.warningTooltip, getI18n(message));
    }
    hideUserChoiceCheckboxAndButtons(window) {
        makeNotVisible(window, Ids.rememberTrustDecisionText);
        makeNotVisible(window, Ids.rememberTrustDecisionCheckbox);
        doNotDisplay(window, Ids.trustUrlButton);
        doNotDisplay(window, Ids.untrustUrlButton);
        addButtonClickHandler(window, Ids.trustUrlButton, () => { });
        addButtonClickHandler(window, Ids.untrustUrlButton, () => { });
    }
    show() {
        const window = this.window;
        const model = this.model;
        const controller = this.controller;
        const isLaunchedPage = maybe_some(model.launchedURL);
        if (maybe_some(model.learnMoreURL)) {
            setElementHref(window, Ids.learnMoreLink, model.learnMoreURL.toString());
            setElementTextContent(window, Ids.learnMoreLinkText, getI18n(I18nMessages.blockedPageHelpLink));
        }
        else {
            doNotDisplay(window, Ids.learnMoreLink);
        }
        if (maybe_some(model.hostnameExplanation)) {
            setElementTextContent(window, Ids.hostnameExplanation, model.hostnameExplanation);
        }
        if (maybe_some(model.launchedURL)) {
            this.setHostname(window, model.launchedURL.hostname);
        }
        else if (maybe_some(model.blockedURL)) {
            this.setHostname(window, model.blockedURL.hostname);
        }
        if (isLaunchedPage) {
            this.setWarningTooltip(window, I18nMessages.blockedPageUntrustedTooltip);
        }
        else {
            this.setWarningTooltip(window, I18nMessages.blockedPageWarningTooltip);
        }
        setElementTextContent(window, Ids.browserExplanation, getI18n(I18nMessages.blockedPageBrowserExplanation));
        addButtonClickHandler(window, Ids.trustUrlButton, () => controller.onTrustUrl(window));
        if (maybe_some(currentBrowser)) {
            setElementTextContent(window, Ids.trustUrlButton, getI18n(I18nMessagesWithSub.trustUrlButton, browserToDisplayString(currentBrowser)));
        }
        else {
            setElementTextContent(window, Ids.trustUrlButton, getI18n(I18nMessages.trustUrlButton));
        }
        addButtonClickHandler(window, Ids.untrustUrlButton, () => controller.onUntrustUrl(window));
        setElementTextContent(window, Ids.untrustUrlButton, getI18n(I18nMessages.untrustUrlButton));
        const rememberDecisionOnChecked = (isChecked) => {
            controller.onRememberTrustDecision(isChecked);
        };
        addCheckboxChangeHandler(window, Ids.rememberTrustDecisionCheckbox, rememberDecisionOnChecked);
        setCheckbox(window, Ids.rememberTrustDecisionCheckbox, model.rememberDecisionsDefault);
        setElementTextContent(window, Ids.rememberTrustDecisionText, getI18n(I18nMessages.rememberTrustDecisionText));
        if (isLaunchedPage) {
            this.hideUserChoiceCheckboxAndButtons(window);
            if (maybe_some(model.launchedPageOpenedSecureExplanation)) {
                setElementTextContent(window, Ids.openedSecureExplanation, model.launchedPageOpenedSecureExplanation);
            }
            if (model.showLaunchedPageButtons && window.history.length > 1) {
                addButtonClickHandler(window, Ids.goBackButton, () => window.history.back());
                setElementTextContent(window, Ids.goBackButton, getI18n(I18nMessages.launchedPageGoBack));
            }
            else {
                doNotDisplay(window, Ids.goBackButton);
            }
            if (maybe_some(model.tabCount) && model.tabCount > 1) {
                addButtonClickHandler(window, Ids.closeTabButton, () => controller.onCloseTab());
                setElementTextContent(window, Ids.closeTabButton, getI18n(I18nMessages.launchedPageCloseTab));
            }
            else {
                doNotDisplay(window, Ids.closeTabButton);
            }
        }
        else {
            doNotDisplay(window, Ids.goBackButton);
        ...
        makeVisible(window, Ids.boundingBox);
    }
    updateViewAfterBlocking() {
        const window = this.window;
        const model = this.model;
        this.hideUserChoiceCheckboxAndButtons(window);
        if (maybe_some(model.blockedPageOpenedSecureExplanation)) {
            setElementTextContent(window, Ids.openedSecureExplanation, model.blockedPageOpenedSecureExplanation);
        }
    }
}

;// CONCATENATED MODULE: ./scripts/blocked-page/main.ts
...
function main(window) {
    initChromeRuntime();
    const options = parseUrlOptions(window);
    const model = new Model(options.navigateToURL, options.blockedURL, options.contentType, options.rememberDecisionsDefault, options.learnMoreURL, options.launchedURL, options.showLaunchedPageButtons);
    let view;
    const show = () => view === null || view === void 0 ? void 0 : view.show();
    const updateViewAfterBlocking = () => view === null || view === void 0 ? void 0 : view.updateViewAfterBlocking();
    const controller = new Controller(model, show, updateViewAfterBlocking);
    view = new View(controller, model, window);
}
window.onload = (event) => {
    main(window);
};
