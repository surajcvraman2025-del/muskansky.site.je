...
function isBrowserUrl(url) {
    const browserSchemes = [
        scheme_Scheme.CHROME,
        scheme_Scheme.ABOUT,
        scheme_Scheme.EDGE
    ];
    return browserSchemes.some((browserScheme) => string_utils_compareStrings(url.protocol, browserScheme));
}
...
;// CONCATENATED MODULE: ./scripts/external-app/external-app-link-controller.ts
...
class ExternalAppLinkController {
    constructor(document, linkSpec, externalAppName, vmid) {
        this.document = document;
        this.linkSpec = linkSpec;
        this.externalAppName = externalAppName;
        this.vmid = vmid;
        this.hadResponse = false;
        this.extensionPortController = new ExtensionPortController(hostConstants.externalAppLinkPagePortName, tabId => this.onExtensionReady());
        this.extensionPortController.registerMessagePayloadHandler(MessageType.externalAppLinkResponseV1, (message) => this.handleResponse(message));
        this.extensionPortController.connect();
        setTimeout(() => {
            if (!this.hadResponse) {
                log_logError("Timed out waiting for a response. Will proceed to the original link URL");
                this.navigate();
            }
        }, 30000);
    }
    navigate() {
        log_log(`ExternalAppLinkController.navigate: ${this.linkSpec}`);
        const url = url_utils_parseUrl(this.linkSpec);
        if (maybe_none(url) || isBrowserUrl(url)) {
            navigateDocument(this.document, "about:blank");
        }
        else {
            navigateDocument(this.document, this.linkSpec);
        }
    }
    onExtensionReady() {
        this.extensionPortController.sendMessage(MessageType.externalAppLinkRequestV16, new ExternalAppLinkRequestV16(this.linkSpec, this.externalAppName, this.vmid));
    }
    handleResponse(ack) {
...
function findURLDocumentQueryParam(documentUrl, queryParam) {
    ...
    if (none(queryParamValue)) {
        return undefined;
    }
    return parseUrl(queryParamValue);
}

;// CONCATENATED MODULE: ../lib/host/external-app-link-page-options.ts
var OptionNames;
(function (OptionNames) {
    OptionNames["linkSpec"] = "linkSpec";
    OptionNames["externalAppName"] = "externalAppName";
    OptionNames["vmid"] = "vmid";
})(OptionNames || (OptionNames = {}));

;// CONCATENATED MODULE: ./scripts/external-app/main.ts
...
function parseOptions(window) {
    ...
    return {
        linkSpec: findDocumentQueryParam(documentURL, OptionNames.linkSpec),
        externalAppName: findDocumentQueryParam(documentURL, OptionNames.externalAppName),
        vmid: findDocumentQueryParam(documentURL, OptionNames.vmid)
    };
}
...
function main(window) {
    ...
    if (maybe_none(options.linkSpec) || maybe_none(options.externalAppName)) {
        navigateDocument(document, (_a = options.linkSpec) !== null && _a !== void 0 ? _a : "about:blank");
        return;
    }
    document.title = options.linkSpec;
    const controller = new ExternalAppLinkController(document, options.linkSpec, options.externalAppName, options.vmid);
}
window.onload = (event) => {
    main(window);
};
